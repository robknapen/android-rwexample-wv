...
public class RWExampleWebViewsActivity extends Activity {
    ...
    private TextView mHeaderLine1TextView;
    ...
    private BroadcastReceiver rwReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (RW.SESSION_ON_LINE.equals(intent.getAction())) {
                ...
                if (mHeaderLine2TextView != null) {
                    mHeaderLine2TextView.setText(mRwBinder.getConfiguration().getProjectName());
                }
                updateUIState(mConnected);
            } else if (RW.NO_CONFIGURATION.equals(intent.getAction())) {
                if (mHeaderLine2TextView != null) {
                    mHeaderLine2TextView.setText(R.string.no_project_info);
                }
                updateUIState(false);
                ...
                String contentFileName = mRwBinder.getContentFilesDir() + "home.html";
                try {
    ...
    private void startRWService(Intent intent) {
        ...
        try {
            ...
            if ((serverName != null) && (serverPage != null)) {
                mRwService.putExtra(RW.EXTRA_SERVER_URL_OVERRIDE, serverName + "/" + serverPage);
            }

            // web content downloading customizations
            mRwService.putExtra(RW.EXTRA_WEB_CONTENT_ALWAYS_DOWNLOAD,
                    prefs.getBoolean(RWPreferenceActivity.ALWAYS_DOWNLOAD_WEB_CONTENT, false));
            mRwService.putExtra(RW.EXTRA_WEB_CONTENT_EXTERNAL_STORAGE,
    ...
    private void initUIWidgets() {
        mHeaderLine1TextView = (TextView) findViewById(R.id.header_line1_textview);
    ...
    private void updateUIState(boolean connectedState) {
        mConnected = connectedState;

        if (mListenButton != null) {
            mListenButton.setEnabled(mConnected);
        }
        ...
        if ((mRwBinder != null) && (mRwBinder.isConnected())) {
            version = mRwBinder.getConfiguration().getServerVersion();
        ...
        mHeaderLine1TextView.setText(version);
    }
